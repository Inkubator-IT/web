...
import { ANCHOR_HEIGHT, type DeskLayout } from "../config";
...
export function LabelTracker({ layout }: { layout: DeskLayout }) {
  const { hoveredId, labelRef, hotspotsRef } = useDesk();
  const camera = useThree((state) => state.camera);
  const size = useThree((state) => state.size);
  const anchor = useMemo(() => new THREE.Vector3(), []);

  const project = (slotId: string, extraHeight: number) => {
    const slot = layout.slots.find((candidate) => candidate.id === slotId);
    if (!slot) return null;

    anchor.set(
      slot.position[0],
      (ANCHOR_HEIGHT[slotId] ?? 0.3) + extraHeight,
      slot.position[2],
    );
  ...
  useFrame(() => {
    ...
    if (label && hoveredId) {
      const point = project(hoveredId, 0.09);
      if (point) {
